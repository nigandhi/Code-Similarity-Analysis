 ...
 * ====================
 * ver 2.4 : 26 March 14
 * - minor changes to some function in order to accept element type 
 *   nodes from the Action and Rules.
 * ver 2.3 : 05 Jan 13
...
namespace TMTree
{
  ...
  public:
    ...
    virtual bool operator()(Node* pNode)
    {
		...
		std::cout << "\n  " << pNode->value().filename << " " << pNode->value().type << " " << pNode->value().name << " " << pNode->value().lineCount<<" "<<pNode->value().complexity;
  ...
  public:
    ...
    Node* find(int value, Node* pNode = 0);
    Node* finder(int value, Node* pNode = 0);
    void clear();
    bool& verbose();
    bool isEmpty();
  private:
  ...
  {
  public:
    finderOp(int val, Node* pNode=0)
      : value_(val), pNode_(pNode), stop(false), verbose_(false) {}
    bool operator()(Node* pNode)
    {
      if(stop) return true;
      if(verbose())
        std::cout << "\n -- value = " << pNode->value().id;
      if(pNode->value().id == value_)
      { 
        pNode_ = pNode;
        return (stop = true);
      }
  ...
  template <typename Node>
  Node* MTree<Node>::finder(int value, Node* pNode=0)
  {
  ...
  template <typename Node>
  Node* MTree<Node>::find(int tvalue, Node* pNode)
  {
    if(pNode == 0)
    {
      pNode = pRoot;
